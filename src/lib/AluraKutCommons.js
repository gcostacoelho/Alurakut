          ...
          <li key={`orkut__icon_set__${slug}`}>
            ...
            <span className="OrkutNostalgicIconSet__iconComplex" className="OrkutNostalgicIconSet__number" style={{ gridArea: 'number' }}>
              {[0, 1, 2].map((_, index) => {
                ...
                return <img key={`orkut__icon_set__${slug}_img_${index}`} src={`https://alurakut.vercel.app/icons/${icon}.svg`} style={{ marginRight: '2px', opacity: isHeartActive ? 1 : '0.7' }} />
              })}
            </span>
          </li>
        );
...
const AlurakutLoginScreen = css`
  :root {
    ...
    --backgroundTertiary: #4F4F4F;
    --backgroundQuarternary: #8f8f8f;
    --colorPrimary: #D02090;
